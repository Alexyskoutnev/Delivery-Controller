...
from RL_Pizza_Delivery.visual.assets import COLOR
...
class ENV_OBSTACLE(ENV_BASE):
    ...
    def reset(self):
        # super(ENV_BASE, self).reset(seed=self.seed)
    ...
    def step(self, action):
        ...
        goal_reward = REWARDS.AT_GOAL if done else 0.0
        reward = -np.sqrt((self.current_pos[0] - self.goal_pos[0])**2 + (self.current_pos[1] - self.goal_pos[1])**2) + pothole_reward + goal_reward
        #==============REWARDS================================#
        if self.render_mode == 'human':
            self._render_frame()
