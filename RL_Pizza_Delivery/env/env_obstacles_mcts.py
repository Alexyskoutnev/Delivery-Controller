...
class ENV_OBSTACLE(ENV_BASE):
    ...
    def _get_observation(self):
        ...
        for traffic_jam in self._traffic_jams:
            if np.all(traffic_jam == self.current_pos):
    ...
    def is_terminal(self):
        ...
        """
        # Define your termination conditions based on the state
        # For example, check if the agent has reached the goal or if a maximum number of moves is reached.
        return np.array_equal(self.current_pos, self.goal_pos) or self._max_times <= self._timestep
